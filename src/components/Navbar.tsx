...
export function Navbar({ onApiConfigSaved }: NavbarProps) {
  ...
  const [apiEndpoint, setApiEndpoint] = useState(
    () => localStorage.getItem('gemini_endpoint') || 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent'
  );
    ...
    <>
      <header className="flex items-center justify-between px-6 py-3 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-sm sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-zinc-100 tracking-tight">JustOne <span className="text-zinc-500 font-normal">AI</span></span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setClearModalOpen(true)} className="btn-ghost text-zinc-500">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
      ...
      <Modal isOpen={apiModalOpen} onClose={() => setApiModalOpen(false)} title="API 配置">
        <div className="space-y-4">
          <div>
            ...
            />
            <p className="text-xs text-zinc-600 mt-1">API Key 仅存储在本地 localStorage，不会上传到任何服务器。</p>
          </div>
          <div>
